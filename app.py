...
.plate {{ font-family:'Anton',sans-serif; font-size:5rem; line-height:.9;
  letter-spacing:.01em; text-transform:uppercase; color:{PAPER}; margin:0; }}
.byline {{ font-family:'Space Mono',monospace; font-size:.72rem; letter-spacing:.16em;
  text-transform:uppercase; color:{PAPER}; opacity:.62; margin:.9rem 0 0; }}
...
def preview(name, artist):
    ...
    q = urllib.parse.urlencode({"term": f"{name} {artist}", "entity": "song", "limit": 1})
    try:
        with urllib.request.urlopen(f"https://itunes.apple.com/search?{q}", timeout=6) as r:
            hits = json.load(r).get("results") or []
        return hits[0].get("previewUrl") if hits else None
    except Exception:
        return None
...
st.markdown(
    f'<div class="hero">'
    f'<p class="eyebrow" style="color:#7FB2E8">Pressing · Side A</p>'
    f'<p class="plate">Can you <em>hear</em><br>a hit?</p>'
    f'<p class="byline">Built by {html.escape(DEVELOPER)}</p>'
...
with right:
    ...
    picks = cand[np.argsort(((Z[cand] - q) ** 2).sum(1))[:3]]
    for i in picks:
        st.markdown(
            ...
            unsafe_allow_html=True)
        url = preview(str(ref.track_name[i]), str(ref.track_artist[i]))
        if url:
            st.audio(url)
        else:
            st.markdown(
...
st.markdown(
    ...
    f'playlists across six genres. Only 6 of the 28,356 tracks are by Indian artists, so '
